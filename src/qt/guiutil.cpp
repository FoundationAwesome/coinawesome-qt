...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("coinawesome"))
        return false;
...
{
    // Convert coinawesome:// to coinawesome:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("coinawesome://"))
    {
        uri.replace(0, 12, "coinawesome:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "CoinAwesome.lnk";
}
...
{
    return GetAutostartDir() / "coinawesome.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("CoinAwesome-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  coinawesome-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("CoinAwesome-Qt"));
